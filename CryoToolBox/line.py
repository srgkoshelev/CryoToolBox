...
class Line:
    ...
    def dP_global(self, fluid, mFlow_in):    #with temp variation
        ...
        for comp in self.compList:
            ...
            if i > 0 and comp.ID > D:
                enl = piping.Enlargement(D, comp.ID)
                dP = piping.dP_dyn(mFlow_in, fluid_temp, enl)
                fluid_temp.update('P', fluid_temp.P - dP,'Hmass', H)
            if i > 0 and comp.ID < D:
                con = piping.Contraction(D, comp.ID)
                dP = piping.dP_dyn(mFlow_in, fluid_temp, con)
                fluid_temp.update('P', fluid_temp.P - dP,'Hmass', H)

            if comp.type == 'Valve':
                dP = piping.dP_dyn(mFlow_in, fluid_temp, comp)
                self.Q_out_comp[i] = 0 * ureg.W/ureg.m**2
                self.Tw_i_comp[i] = self.Tw_o_comp[i] = fluid_temp.T
                dH = 0*ureg.J/ureg.kg
            else:
                Tw_i, Tw_o, dP, Q = heated_pipe.pipe_heat(comp, fluid_temp, mFlow_in) #change to dP, and dH
                self.Tw_i_comp[i] = Tw_i.m_as(ureg.K) * ureg.K
            ...
            dP_hydro = piping.dP_hydro(fluid_temp, comp)
            fluid_temp.update('P', fluid_temp.P - dP - dP_hydro,'Hmass', H + dH.to(ureg.J/ureg.kg))
            self.P_out_comp[i] = fluid_temp.P
            self.T_out_comp[i] = fluid_temp.T
