CryoToolBox provides utility functions for variety of engineering calculations, including fluid dynamic, heat transfer, and oxygen deficiency analyses. The setup is simple:
#+begin_src python
  >>> import CryoToolBox as ctb
  >>> u = ctb.ureg  # Unit registry that handles unit conversions
#+end_src

Easy to create and get dimensions of pipes and tubes. Inputs in different units is supported:
#+begin_src python
...
Under the hood CoolProp calculates fluid properties, no matter what units are used to define the thermodynamic state:
#+begin_src python
...
Additionally, pressure drop calculations for other piping elements, e.g., elbows, tees, or valves, are supported:
#+begin_src python
...
The package also supports calculation of pressure drops for incompressible flow, isothermal or adiabatic flows. Here's an example of calculation for helium running through 1" NPS pipe (both defined above):
#+begin_src python
...
CryoToolBox can calculate some other useful quantities for manual like checks, e.g., Reynolds number or Darcy friction factor:
#+begin_src python
...
The package also supports pressure relief sizing, for example API-520 recommended direct integration method.
#+begin_src python
