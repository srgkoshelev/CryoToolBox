...
def approx_equal(expected, calculated, tol):
    return abs(expected-calculated)/expected < tol
...
def dP_Mach(m_dot, fluid, pipe):
    M = Mach_total(fluid, m_dot, pipe.area)
    # print('Mach at inlet: ', M)
    K_lim = K_limit(M, fluid.gamma)
    # print('K limit: ', K_lim)
    # print('K pipe: ', pipe.K(Re).to_base_units())
    Re = ht.Re(fluid, m_dot, pipe.ID, pipe.area)
    # check(M, M_from_K_limit(K_lim, air.gamma))
    K_left = K_lim - pipe.K(Re)
    if K_left < 0:
        raise ValueError('Flow is choked. Reduce hydraulic resistance or mass flow.')
    # print('K left: ', K_left)
    M_end = M_from_K_limit(K_left, fluid.gamma)
    # print('M end: ', M_end)
    P_static_end = P_from_M(fluid.P, M, M_end, fluid.gamma)
    # print('P static end: ', P_static_end)
    P_total_end = P_total_from_static(P_static_end, M, fluid.gamma)
    ...
    for method in methods:
        try:
            solution = root_scalar(to_solve_sq, x0=x0, x1=x1, fprime=True,
                                    fprime2=True, bracket=bracket, method=method)
            P_2_root_sq = solution.root**0.5 * ureg.Pa
            ht.logger.debug(method, solution.iterations)
        except TypeError:
            ht.logger.debug(f'{method} method failed for square solve')
            P_2_root_sq = None
...
for i in range(100):
    M_total_test = Mach_total(ht.Air, 100*u.g/u.s, 1*u.inch**2)
end = time.perf_counter()
print(f'M_total routine time: {(end-start)*10:.4g} ms')

start = time.perf_counter()
for i in range(100):
    M_from_K_limit(20, 1.4)
end = time.perf_counter()
print(f'M_from_K_limit routine time: {(end-start)*10:.4g} ms')
